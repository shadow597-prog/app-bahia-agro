...
# Link da sua planilha (opcional, se quiser enviar os dados para lá também)
...
def main(page: ft.Page):
    ...
    page.padding = 30
    page.bgcolor = "#0f172a" # Cor de fundo do seu código original
    
    # Cores do seu código CustomTkinter
    COR_CACAU = "#8B4513"
    COR_BANANA = "#EAB308"
    COR_FUNDO_CARD = "#1e293b"

    def salvar_dados(cultura, var, proc, qtd, uni):
        # Aqui ele envia para a planilha (se o link estiver configurado)
        ...
        page.snack_bar = ft.SnackBar(ft.Text(f"✅ {cultura} gravado com sucesso!"), bgcolor="green")
        page.snack_bar.open = True
        page.update()

    # --- UI: CABEÇALHO ---
    header = ft.Text("SISTEMA GESTÃO DE SAFRA", size=32, weight="bold", color="white")

    # --- UI: ABA LANÇAMENTO ---
    # Coluna Cacau
    var_cacau = ft.Dropdown(label="Variedade", options=[ft.dropdown.Option("PS-1319"), ft.dropdown.Option("CCN-51"), ft.dropdown.Option("Comum")])
...
* COMUM: Sabor chocolate intenso (Cabruca).

[ BANANA ]
* PRATA: Melhor durabilidade pós-colheita.
* NANICA: Porte baixo, muito doce.
* TERRA: Grande, para cozinhar/fritar.
    """
    ...
    tabs = ft.Tabs(
        ...
        tabs=[
            ft.Tab(text="📊 Planilhas", content=ft.Text("Os dados estão sendo salvos na sua Planilha Google!")),
            ft.Tab(text="➕ Novo Lançamento", content=aba_lancamento),
            ft.Tab(text="📖 Conhecimento", content=aba_conhecimento),
        ],
        expand=1
    )
